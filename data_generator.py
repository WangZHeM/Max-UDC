...
# 用户数据
def generate_userdata(user_n, pois):
    np.savetxt("userpoi.txt", [], fmt="%d")
    np.savetxt("usertimes.txt", [], fmt="%d")
    ...
    for i in range(0, user_n):
        ...
        for p in temp:
            while 1:
                ...
                if p[1] > map_width:
                    p[1] = map_width
                if p not in user_poi:
                    user_poi.append(p)
                    break
        print(user_poi)
        # 设置用户起点坐标
        user_index = random.choice(user_poi)
        # 设置路径
        temp = copy.deepcopy(user_index)
        temp_poi = copy.deepcopy(user_poi)
        times = 0
        ...
        user_times = np.random.normal((max_time - times) // user_poiNum, scale=3, size=user_poiNum)
        user_times = list(user_times)
        for tt in range(len(user_times)):
            if user_times[tt] < 1:
                user_times[tt] = 1
        with open('userpoi.txt', 'a') as f1:
            np.savetxt(f1, user_poi, delimiter=',', newline=' ', fmt='%d')
            f1.write('\n')
        f1.close()
        with open('usertimes.txt', 'a') as f2:
            np.savetxt(f2, user_times, delimiter=',', newline=' ', fmt="%d")
            f2.write('\n')
        f2.close()
...
if __name__ == '__main__':
    ...
    # # np.save("userpoi.txt", userpoi, fmt="%d")
    # np.savetxt("usertimes.txt", userti, fmt="%d")
    # np.savetxt("poitimes.txt", poi_uavned, fmt="%d")
